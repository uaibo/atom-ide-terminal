...
class PlatformIOTerminalView extends View
  ...
  @content: ->
    @div class: 'atom-ide-terminal terminal-view', outlet: 'platformIOTerminalView', =>
  ...
  initialize: (@id, @pwd, @statusIcon, @statusBar, @shell, @args=[], @env={}, @autoRun=[]) ->
    ...
    @prevHeight = atom.config.get('atom-ide-terminal.style.defaultPanelHeight')
    ...
    @setAnimationSpeed()
    @subscriptions.add atom.config.onDidChange 'atom-ide-terminal.style.animationSpeed', @setAnimationSpeed

    override = (event) ->
      return if event.originalEvent.dataTransfer.getData('atom-ide-terminal') is 'true'
    ...
    @xterm.on 'mouseup', (event) =>
      if event.which != 3
        ...
        if atom.config.get('atom-ide-terminal.toggles.selectToCopy') and text
    ...
    if /zsh|bash/.test(@shell) and @args.indexOf('--login') == -1 and Pty.platform isnt 'win32' and atom.config.get('atom-ide-terminal.toggles.loginShell')
      @args.unshift '--login'
  ...
  setAnimationSpeed: =>
    @animationSpeed = atom.config.get('atom-ide-terminal.style.animationSpeed')
  ...
  displayTerminal: ->
    ...
    @terminal = new Terminal {
      cursorBlink     : false
      scrollback      : atom.config.get 'atom-ide-terminal.core.scrollback'
      cols, rows
    }
  ...
  attachListeners: ->
    @ptyProcess.on "atom-ide-terminal:data", (data) =>
      @terminal.write data

    @ptyProcess.on "atom-ide-terminal:exit", =>
      @destroy() if atom.config.get('atom-ide-terminal.toggles.autoClose')
    ...
    @ptyProcess.on "atom-ide-terminal:title", (title) =>
      @process = title
    ...
    @terminal.once "open", =>
      ...
      autoRunCommand = atom.config.get('atom-ide-terminal.core.autoRunCommand')
  ...
  open: =>
    ...
    @onTransitionEnd =>
      if not @opened
        ...
        @emit "atom-ide-terminal:terminal-open"
      else
        @focus()
  ...
  pty: () ->
    if not @opened
      wait = new Promise (resolve, reject) =>
        @emitter.on "atom-ide-terminal:terminal-open", () =>
          resolve()
  ...
  ptyPromise: () ->
    new Promise (resolve, reject) =>
      if @ptyProcess?
        @ptyProcess.on "atom-ide-terminal:pty", (pty) =>
          resolve(pty)
  ...
  applyStyle: ->
    config = atom.config.get 'atom-ide-terminal'
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontFamily', (event) =>
      ...
      @terminal.element.style.fontFamily = overrideFont or editorFont or defaultFont
    @subscriptions.add atom.config.onDidChange 'atom-ide-terminal.style.fontFamily', (event) =>
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontSize', (event) =>
      ...
      @terminal.element.style.fontSize = "#{overrideFontSize or editorFontSize}px"
      @resizeTerminalToView()
    @subscriptions.add atom.config.onDidChange 'atom-ide-terminal.style.fontSize', (event) =>
  ...
  insertSelection: (customText) ->
    ...
    runCommand = atom.config.get('atom-ide-terminal.toggles.runInsertedText')
    selectionText = ''
  ...
  getTitle: ->
    @statusIcon.getName() or "atom-ide-terminal"
